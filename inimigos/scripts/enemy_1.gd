...
var powerUp 		:PackedScene			= preload("res://inimigos/drop_itens/powerUp.tscn")
...
func _physics_process(delta):
	if Global.dist_to_player(self, Global.playerPosition) >= 1024:
		self.queue_free()
	if Global.dist_to_player(self, Global.playerPosition) <= 120:
		enable_colisions()
	else:
		disable_colisions()
	if canFollow:
...
func hit():
	if Global.canHited and player != null and not died:
		$bow.par = player.global_position
